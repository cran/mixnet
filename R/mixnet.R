mixnet<-function(x,qmin=2,qmax=NULL,method="variationnal",nbiter=10,improve=FALSE)
  {
...
### How the graph is coded ?
  ...
  if (method=="bayesian"){
    cat("Bayesian estimation method is not implemented yet \n")
    }
  ...
  cur <- 1
  for (i in span){
    ## format : y[[j]]$name <- dataFormat(x$res[cur:end]); cur <- (offset equal to the size of dataFormat)
    y[[j]]$ICL    <- xout$res[cur]; cur <- cur+1
    y[[j]]$alphas <- xout$res[cur:(cur-1+i)]; cur <- cur+i
    y[[j]]$Pis    <- matrix(xout$res[cur:(cur-1+(i*i))], i,i); cur <- cur+(i*i)
    y[[j]]$Taus   <- matrix(xout$res[cur:(cur-1+(i*nbrNodes))], i, nbrNodes,byrow=TRUE); cur <- cur+(i*nbrNodes)
    y[[j]]$Taus[,readingOrder]<-y[[j]]$Taus
    j <- j+1
  }
  result<-list(edges=m,qmin=qmin,qmax=qmax,output=y)
  class(result)<-"mixnet"
  return(result)
}
############################################################
...
ploticl<-function(x,q,...)
  { 
    ...
    ICL<-unlist(lapply(x$output,ICL<-function(x) x$ICL))
    plot(Q,ICL,xlab="Number of classes",ylab="ICL",main="Integrated Classification Likelihhod")
    lines(Q,ICL)
...
############################################################
is.mixnet<-function(x){if (class(x)=="mixnet") TRUE else FALSE}
...
if (is.null(q)) {# find the best number of classes according ICL
                  ICL<-unlist(lapply(mixnet.res$output,ICL<-function(x) x$ICL))
                  which.max(ICL)->i
                  q<-length(mixnet.res$output[[i]]$alphas)
                  }
